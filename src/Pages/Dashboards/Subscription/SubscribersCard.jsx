...
const SubscribersCard = ({ data }) => {
  const { earning, subscription_count, active_subscriber_count } = data || {};
  ...
  return (
    <div className="grid grid-cols-3 gap-10 mb-8">
      <div className="p-6 rounded-xl shadow-lg border border-gray-200">
        ...
        <p className="text-3xl font-medium mt-4">{subscription_count || 0}</p>
      </div>
      <div className="p-6 rounded-xl shadow-lg border border-gray-200">
        <div className="flex justify-between items-center">
        ...
        <p className="text-3xl font-medium mt-4 text-[#019701]">${earning || 0}</p>
      </div>
      <div className="p-6 rounded-xl shadow-lg border border-gray-200">
        <div className="flex justify-between items-center">
          <p className="text-xl font-medium opacity-80">Active Subscribers</p>
          <Eye className="w-5 h-5 text-[#B8860B]" />
        </div>
        <p className="text-3xl font-medium mt-4">{active_subscriber_count || 0}</p>
      </div>
    </div>
  );
};
